...
public class bulletControl : MonoBehaviour
{
    ...
    public void kill(){if(!this||!gameObject)return;HUDHandler.PlaySfx(boom);var camera=GameObject.Find("Main Camera");var player=GameObject.Find("Character");if(camera&&player){var shake=camera.GetComponent<cameraScript>();if(shake){shake.shakeTime=5f/Mathf.Max(.01f,Vector3.Distance(body.position,player.transform.position));shake.currShakeTime=0;}}

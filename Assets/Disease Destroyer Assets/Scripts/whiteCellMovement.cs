...
public class whiteCellMovement : MonoBehaviour
{
    ...
    public void kill(){health--;if(visual){if(health==2&&tex2)visual.material.mainTexture=tex2;if(health==1&&tex3)visual.material.mainTexture=tex3;}if(health<=0){HUDHandler.Explode(transform.position,new Color(.3f,.8f,1f),1.2f);if(emitter){emitter.transform.SetParent(null);emitter.Play();Destroy(emitter.gameObject,3);}if(splat&&!oneShotAudio){HUDHandler.PlaySfx(splat.clip,splat.volume);oneShotAudio=true;}Destroy(gameObject);}}
